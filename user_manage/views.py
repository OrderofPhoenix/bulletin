...
def index(request):
    if request.user.is_authenticated():
        return render(request, 'info.html', {'msg': 'Welcome, ' + request.user.username})
    ...
    if request.method == 'POST':
        ...
        if login_form.is_valid():
            ...
            if User.objects.get_by_natural_key(uid).is_active == False:
                return render(request, 'info.html', {'msg': 'your account is not activated yet!', 'not_activate': True,
                                                     'uid': uid})
            pwd = login_form.cleaned_data['pwd']
            user = authenticate(username=uid, password=pwd)
            if user is not None:
                login(request, user)
                return render(request, 'info.html', {'msg': 'Welcome, ' + uid})
            else:
                return render(request, 'info.html', {'msg': 'username or password wrong!'})
        else:
            return render(request, 'index.html', {'login_form': login_form})
    else:
        login_form = LoginForm()
...
def sign_up(request):
    if request.method == 'POST':
        ...
        if reg_form.is_valid():
            ...
            if uid in uid_list:
                return render(request, 'info.html', {'msg': 'user is already exists!'})
            ...
            if pwd != re_pwd:
                return render(request, 'info.html', {'msg': 'two passwords do not match!'})
            user = User(username=uid, email=email,
                        security_question=sec_q, security_answer=sec_a, is_active=False)
            user.set_password(pwd)
            user.save()
            return render(request, 'info.html', {'msg': 'registration complete!'})
    else:
...
def verify_uid(request):
    if request.method == 'POST':
        ...
        if User.objects.get(username=uid) is not None:
            ...
            return render(request, 'find_password.html', {'fp_form': fp_form, 'user': user})
        else:
            return render(request, 'info.html', {'msg': 'user does not exist!'})
    else:
        fp_form = FindPasswordForm()
        return render(request, 'verify_uid.html', {'fp_form': fp_form})
    ...
    if request.method == 'POST':
        ...
        if fp_form.is_valid():
            ...
            if sec_a == user.security_answer:
                ...
                return render(request, 'modify_password.html', {'rp_form': rp_form, 'is_verified': True, 'user': user})
            else:
                return render(request, 'info.html', {'msg': 'security answer is not current!'})
        else:
            fp_form = FindPasswordForm()
            return render(request, 'find_password.html', {'fp_form': fp_form})
    else:
        fp_form = FindPasswordForm()
        return render(request, 'find_password.html', {'fp_form': fp_form})
    ...
    if request.method == 'POST' and request.POST.get('type') == 'modify':
        ...
        if mp_form.is_valid():
            ...
            if user:
                if pwd == re_pwd:
                    ...
                    request.user.save()
                    return render(request, 'info.html', {'msg': 'password has been successfully modified!'})
                else:
                    return render(request, 'info.html', {'msg': 'two passwords do not match!'})
            else:
                return render(request, 'info.html', {'msg': 'old password wrong!'})
        else:
        ...
        if rp_form.is_valid():
            ...
            if pwd == re_pwd:
                user.set_password(pwd)
                user.save()
                return render(request, 'info.html', {'msg': 'your password has successfully reset'})
            else:
                return render(request, 'info.html', {'msg': 'two passwords do not match!'})
        else:
            user = User.objects.get_by_natural_key(request.POST.get('username'))
...
def self_profile(request):
    if request.user is None:
        ...
        return render(request, 'info.html', {'msg': 'PROFILE\nusername:' + user.username
                                                    + '\nemail:' + user.email
...
def activate_mail_send(request, username):
    ...
    message = 'To activate your account go to '+ DOMAIN +'/u/activate='+token
    send_mail(subject='Account Activation', message=message,from_email='team_phoenix@outlook.com' ,recipient_list=[email])
    return render(request, 'info.html', {'msg': 'activation has been sent!'})

def activate(request, token):
    try:
        username = token_confirm.confirm_validate_token(token)
    except:
        return render(request, 'info.html', {'msg': 'verify link has expired!'})

    try:
        user = User.objects.get_by_natural_key(username)
    except:
        return render(request, 'info.html', {'msg': 'user you request is not exist!'})
    user.is_active=True
    user.save()
    return render(request, 'info.html', {'msg': 'you have successfully activated your account!'})
